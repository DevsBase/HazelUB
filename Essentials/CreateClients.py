...
class CreateClients:
  def __init__(self, other):
    self.config = other.output
    API_ID, API_HASH = self.config.get('API_ID'), self.config.get('API_HASH')
    PYROGRAM_SESSION, BOT_TOKEN = self.config.get("PYROGRAM_SESSION"), self.config.get("BOT_TOKEN")
    MONGO_DB_URL = self.config.get('MONGO_DB_URL')
    ...
    add_client(self.app)
    for i in self.config.get('OtherSessions'):
