...
@Tele.on_message(filters.command("purge") & filters.group, fsudo=True)
async def purgeFunc(app: Client, m: Message):
    ...
    for x in range(start, end + 1, 100):
        try:
            x_range = list(range(x, x+101))
            count += await app.delete_messages(m.chat.id, x_range, revoke=True) # type: ignore
